...
function switchLeaderboard(period, btn) {
  ...
  document.getElementById("leaderboard-body").innerHTML = data
    .map(
      (row) => `
    <tr class="clickable-row" onclick="viewUserProfile('${row.user}','${row.initials}')">
      <td>${rankIcon(row.rank)}</td>
      <td>
        <div class="flex items-center gap-2">
          <div class="rank-avatar" style="${rankColor(row.rank)}">${row.initials}</div>
          <span class="font-semibold" style="color:var(--fg)">${row.user}</span>
        </div>
      </td>
