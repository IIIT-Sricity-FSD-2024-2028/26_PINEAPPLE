...
(function () {
  ...
  function ensureUserScoreProfile(userRecord, userName) {
    if (!userRecord) return { xp: 0, rep: 0 };
  ...
  function getApprovedMentorAssignment(project, runtime = getProjectRuntime(project)) {
    const mentorRequest = runtime?.mentorRequest || null;
    if (String(mentorRequest?.status || "").trim().toLowerCase() !== "approved") {
      return null;
    }
    const users =
      typeof getStateUsersStore === "function" ? getStateUsersStore() : {};
    const mentorName = String(mentorRequest.requestedName || "").trim();
    const mentorRecord = mentorName ? getUserRecordByName(mentorName, users) : null;
    const mentorEmail = normalizeEmailValue(
      mentorRequest.mentorEmail || mentorRecord?.email || "",
    );
    return {
      name: mentorName,
      email: mentorEmail,
      ownerEmail: normalizeEmailValue(
        mentorRequest.ownerEmail || getProjectOwnerEmail(project, users),
      ),
      request: mentorRequest,
    };
  }

  function currentUserHasMentorAccess(project, runtime = getProjectRuntime(project)) {
    const assignment = getApprovedMentorAssignment(project, runtime);
    if (!assignment) return false;
    const currentName = String(getCurrentUserName() || "").trim().toLowerCase();
    const mentorName = String(assignment.name || "").trim().toLowerCase();
    const currentEmail = normalizeEmailValue(
      typeof getCurrentUserSessionEmail === "function"
        ? getCurrentUserSessionEmail()
        : "",
    );
    return (
      (mentorName && mentorName === currentName) ||
      (assignment.email && assignment.email === currentEmail)
    );
  }

  function removeLegacyMentorRequests(project) {
  ...
  function revokeProjectMentorAccess(project, options = {}) {
    const runtime = getProjectRuntime(project);
    if (!project || !runtime) return null;
    const assignment = getApprovedMentorAssignment(project, runtime);
    const users =
      typeof getStateUsersStore === "function" ? getStateUsersStore() : {};
    const removedOn = getLiveTimestamp();
    const mentorName = String(
      assignment?.name ||
        runtime.mentorRequest?.requestedName ||
        getProjectMentor(project)?.name ||
        "",
    ).trim();
    const mentorRecord = mentorName ? getUserRecordByName(mentorName, users) : null;
    const mentorEmail = normalizeEmailValue(
      assignment?.email || runtime.mentorRequest?.mentorEmail || mentorRecord?.email || "",
    );
    const ownerEmail = normalizeEmailValue(
      assignment?.ownerEmail ||
        runtime.mentorRequest?.ownerEmail ||
        getProjectOwnerEmail(project, users),
    );
    const normalizedMentorName = mentorName.toLowerCase();

    project.members = (project.members || []).filter((member) => {
      const isMentorRole = String(member.role || "").trim().toLowerCase() === "mentor";
      if (!isMentorRole) return true;
      return (
        normalizedMentorName &&
        String(member.name || "").trim().toLowerCase() !== normalizedMentorName
      );
    });
    project.collaborators = project.members.length;

    runtime.mentorRequest = null;
    runtime.mentorRequestDeleted = options.markDeleted
      ? {
    ...
    saveSharedMentorRequests(
      loadSharedMentorRequests().filter((item) => item.projectId !== project.id),
    );
    removeLegacyMentorRequests(project);

    if (mentorEmail && users[mentorEmail]) {
      removeUserProject(users[mentorEmail], project.id);
      if (options.notifyMentor !== false) {
        const title = options.selfLeave ? "Mentor Access Ended" : "Removed as Mentor";
        const reason = String(options.reason || "").trim();
        pushUserNotification(users[mentorEmail], {
          type: options.selfLeave ? "MENTOR_LEFT" : "MENTOR_REMOVED",
          message: reason
            ? `${project.name}: ${reason}`
            : `Your mentor access for ${project.name} has ended.`,
          projectId: project.id,
          from: ownerEmail,
          status: options.selfLeave ? "left" : "removed",
          timestamp: removedOn,
          icon: options.selfLeave ? "↩" : "🚫",
          title,
          desc: reason
            ? `${project.name}: ${reason}`
            : `Your mentor access for ${project.name} has ended.`,
    ...
    if (options.notifyOwner && ownerEmail && users[ownerEmail]) {
      pushUserNotification(users[ownerEmail], {
        type: "MENTOR_LEFT",
        message: `${mentorName || "The mentor"} left ${project.name}.`,
        projectId: project.id,
        from: mentorEmail,
        status: "left",
        timestamp: removedOn,
        icon: "↩",
        title: "Mentor Left",
        desc: `${mentorName || "The mentor"} left ${project.name}.`,
        time: removedOn,
      });
    }
    ...
    return { mentorName, mentorEmail, ownerEmail, removedOn };
  }
  ...
  const REVIEW2_RUNTIME_VERSION = "3";
  ...
  function hydrateReview2Runtime() {
    try {
      ...
      if (!raw) return;
      const parsed = JSON.parse(raw);
      if (String(parsed?.version || "") !== REVIEW2_RUNTIME_VERSION) {
        localStorage.removeItem(REVIEW2_RUNTIME_KEY);
        return;
      }
  ...
  function persistReview2Runtime() {
    try {
      localStorage.setItem(
        REVIEW2_RUNTIME_KEY,
        JSON.stringify({
          version: REVIEW2_RUNTIME_VERSION,
          projects: PROJECTS,
          applied: APPLIED,
  ...
  function buildSeedTasks(project, members, isCompleted) {
    ...
    const collaborator = actualCollaborator;
    const secondary =
      members.find(
    ...
    const tasks = [
      ...
      },
      {
        ...
        assignee:
          secondary === "Unassigned" ? collaborator : secondary,
        due: "Apr 05",
        priority: "High",
      ...
      },
      {
        ...
        assignee:
          project.id === "7" ? "Vikram Nair" : "Unassigned",
        due: "Apr 08",
        priority: "High",
    ...
    return [
      {
        ...
        by: members[1]?.name || members[0]?.name || project.owner,
        status: "Approved",
        summary: "Delivered the main overview and state cards.",
      },
      {
        title: "Validation pass",
        by: members[2]?.name || members[1]?.name || project.owner,
  ...
  function seedRuntimeData() {
    ...
    const currentUser = getCurrentUserName();
    const isSeedUser = (typeof ADMIN_USERS !== 'undefined' && Array.isArray(ADMIN_USERS) && ADMIN_USERS.some(u => u.name === currentUser)) || 
                       (typeof OTHER_PROFILES !== 'undefined' && OTHER_PROFILES[currentUser]);
                       
    if (!isSeedUser) return; // Only apply mock user-specific data to demo users
    ...
    mount.innerHTML = `
      ...
      <div id="member-kick-modal" class="modal-overlay" onclick="closeKickMemberModal(event)">
        <div class="modal-box" onclick="event.stopPropagation()">
          <div class="modal-header">
            <span class="modal-icon">🚫</span>
            <h3 id="kick-member-title" class="modal-title">Remove Member</h3>
            <button class="modal-close" onclick="closeKickMemberModal()">✕</button>
          </div>
          <div class="modal-body">
            <p id="kick-member-desc" class="text-sm text-muted" style="margin-bottom:12px">Share why this collaborator is being removed. This reason will be shown to them and future join attempts will be blocked.</p>
            <textarea id="kick-member-reason" class="input" rows="4" placeholder="Explain why access is being revoked..."></textarea>
            <button id="kick-member-confirm" class="btn btn-destructive btn-full mt-3" onclick="confirmKickMember()">Remove Member</button>
          </div>
        </div>
      </div>
  ...
  function openKickMemberModal(projectId, memberName, event, targetType = "collaborator") {
    if (event?.stopPropagation) event.stopPropagation();
    ensureOverlayMount();
    ensureReview2State();
    const normalizedTargetType = targetType === "mentor" ? "mentor" : "collaborator";
    STATE.review2.pendingKick = {
      projectId,
      memberName,
      reason: "",
    ...
    const title = document.getElementById("kick-member-title");
    const desc = document.getElementById("kick-member-desc");
    const confirmButton = document.getElementById("kick-member-confirm");
    if (input) input.value = "";
    if (normalizedTargetType === "mentor") {
      if (title) title.textContent = "Remove Mentor";
      if (desc) {
        desc.textContent =
          "Share why this mentor is being removed. This reason will be shown to them and their project workspace access will be revoked.";
      }
      if (confirmButton) confirmButton.textContent = "Remove Mentor";
    } else {
      if (title) title.textContent = "Remove Member";
      if (desc) {
        desc.textContent =
          "Share why this collaborator is being removed. This reason will be shown to them and future join attempts will be blocked.";
      }
      if (confirmButton) confirmButton.textContent = "Remove Member";
    }
    if (modal) modal.classList.add("open");
  }
  ...
  function confirmKickMember() {
    const { projectId, memberName, targetType } = STATE.review2.pendingKick || {};
    const input = document.getElementById("kick-member-reason");
    ...
    if (targetType === "mentor") {
      performRemoveProjectMentor(projectId, reason);
      return;
    }
    const project = PROJECTS.find((item) => item.id === projectId);
    if (!project) return;
  ...
  function performRemoveProjectMentor(projectId, reason) {
    const project = PROJECTS.find((item) => item.id === projectId);
    const runtime = getProjectRuntime(project);
    if (!project || !runtime) {
      showToast("Project not found", "error");
    ...
    if (project.owner !== getCurrentUserName()) {
      showToast("Only the project owner can remove the mentor", "error");
      return;
    }
    const removed = revokeProjectMentorAccess(project, {
      reason,
      markDeleted: true,
      notifyMentor: true,
    });
    if (!removed) {
      showToast("No approved mentor is assigned to this project", "error");
      return;
    }
    closeKickMemberModal();
    persistReview2Runtime();
  ...
  function removeProjectMentor(projectId) {
    ...
    openKickMemberModal(
      projectId,
      assignment.name || "Assigned Mentor",
      null,
      "mentor",
    );
  }

  function leaveMentorProject(projectId) {
    const project = PROJECTS.find((item) => item.id === projectId);
    const runtime = getProjectRuntime(project);
    if (!project || !runtime) {
      showToast("Project not found", "error");
    ...
    if (!currentUserHasMentorAccess(project, runtime)) {
      showToast("You are not the assigned mentor for this project", "error");
      return;
    }
    const left = revokeProjectMentorAccess(project, {
      selfLeave: true,
      notifyMentor: false,
      notifyOwner: true,
    });
    ...
    const wasViewingProject = STATE.selectedProject === projectId;
    persistReview2Runtime();
    if (wasViewingProject) {
      exitProjectWorkspace("mentored-projects");
    }
    renderMentoredProjects();
    renderMentors();
    renderMentorRequests();
    ...
    showToast(`You have left ${project.name} as mentor`);
  }
  ...
  function renderMentoredProjects() {
    ...
    const runtimeItems = PROJECTS.filter((project) => {
      const runtime = getProjectRuntime(project);
      return currentUserHasMentorAccess(project, runtime);
    });
    const runtimeHtml = runtimeItems
  ...
  function openWorkspace(projectId, sourcePage) {
    ...
    const runtime = getProjectRuntime(project);
    if (STATE.role === "mentor" && !currentUserHasMentorAccess(project, runtime)) {
      STATE.selectedProject = "";
      STATE.workspaceMode = "";
      if (typeof saveViewState === "function") saveViewState();
      renderMentoredProjects();
      showToast("You no longer have mentor access to this project", "error");
      navigate("mentored-projects");
      return;
    }
    if (STATE.role === "collaborator" && !canAccessCollaboratorProject(project)) {
      STATE.selectedProject = "";
      STATE.workspaceMode = "";
      if (typeof saveViewState === "function") saveViewState();
      showToast("You no longer have collaborator access to this project", "error");
      openCollaboratorProjectPreview(projectId);
      return;
    }
    STATE.selectedProject = projectId;
  ...
  function renderProjectWorkspace() {
    ...
    const currentUser = getCurrentUserName();
    if (isMentorView && !currentUserHasMentorAccess(project, runtime)) {
      showToast("You no longer have mentor access to this project", "error");
      renderMentoredProjects();
      exitProjectWorkspace("mentored-projects");
      return;
      ...
      <div class="workspace-shell">
        ...
        <div class="card workspace-hero">
          <div>
            ...
            <p class="text-muted">${escapeHtml(project.desc)}</p>
            ${isMentorView && !project.isCompleted ? `<button class="btn btn-ghost btn-sm" style="margin-top:8px" onclick="leaveMentorProject('${project.id}')">Leave as Mentor</button>` : ""}
          </div>
          <div class="workspace-hero__meta">
            <div class="workspace-hero__metric">${Number(project.progress) || 0}%</div>
  ...
  function renderHelp() {
    ...
    const supportCard = document.getElementById("support-category")?.closest('.card');
              ...
              ? completedProjects
                  .map(
                    (project) => `
                      <div class="proj-row">
                        <div class="proj-name">${escapeHtml(project.name)}</div>
                        <div class="proj-meta">${escapeHtml(project.role)} &middot; ${escapeHtml(project.contribution || "")}</div>
                        ${
                          project.finalLink
                            ? `<a class="workspace-link" href="${escapeHtml(project.finalLink)}" target="_blank" rel="noopener noreferrer">Final Link &#8599;</a>`
                            : ""
              ...
              ? mentorRecommendations
                  .map(
                    (badge) => `
                      <div class="proj-row">
                        <div class="proj-name">${escapeHtml(badge.project)}</div>
                        <div class="proj-meta">Recommended by ${escapeHtml(badge.mentor)} &middot; ${escapeHtml(badge.note)}</div>
                      </div>
                    `,
                  )
                  .join("")
            ...
            ? mentoredProjects
                .map(
                  (project) => `
                    <div class="proj-row">
                      <div class="proj-name">${escapeHtml(project.name)}</div>
                      <div class="proj-meta">Owner: ${escapeHtml(project.owner || "Project Owner")} &middot; ${escapeHtml(project.status || project.completedAt || "Completed")}</div>
                      <div class="proj-meta">${escapeHtml(project.contribution || "")}</div>
                      ${
                        project.finalLink
                          ? `<a class="workspace-link" href="${escapeHtml(project.finalLink)}" target="_blank" rel="noopener noreferrer">Final Link &#8599;</a>`
                ...
                (profile.activity || []).length
                  ? profile.activity
                      .map(
                        (activity) => `
                          <div class="activity-row">
                            <div class="activity-icon-wrap">&bull;</div>
                            <div>
                              <div class="activity-action">${escapeHtml(activity.action)}</div>
                              <div class="activity-meta">${escapeHtml(activity.project)} &middot; ${escapeHtml(activity.time)}</div>
                            </div>
                          </div>
                        `,
                      )
  ...
  function renderDashboard() {
    ...
    const historyRows = PROJECTS.filter((project) => {
      ...
      .map((project) => {
        const runtime = getProjectRuntime(project);
        const latestEntry = runtime.contributionHistory.find(
          (item) => item.by === currentUser,
        );
        if (!latestEntry) {
          return "";
