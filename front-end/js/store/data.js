...
function syncRoleData() {
  ...
  if (!localStorage.getItem("teamforge.seededProjects")) {
     // Seed legacy static projects ONLY into their actual owner's data
     PROJECTS.forEach(proj => {
        // Build email from owner name: "Arjun Sharma" -> "arjun.sharma@teamforge.io"
        const nameParts = (proj.owner || "").trim().split(/\s+/);
        const ownerEmail = nameParts.length >= 2
          ? (nameParts[0].toLowerCase() + "." + nameParts.slice(1).join("").toLowerCase() + "@teamforge.io")
          : "";
        if (ownerEmail && users[ownerEmail]) {
           if (!users[ownerEmail].data) users[ownerEmail].data = { projects: [], notifications: [] };
           if (!users[ownerEmail].data.projects) users[ownerEmail].data.projects = [];
           // Only add if not already present
           const alreadyHas = users[ownerEmail].data.projects.some(p => p.id === proj.id);
           if (!alreadyHas) {
             const seedCopy = { ...proj, tasks: [], mentors: [], requests: [], invites: [] };
             users[ownerEmail].data.projects.push(seedCopy);
           }
        }
     });
     localStorage.setItem("users", JSON.stringify(users));
