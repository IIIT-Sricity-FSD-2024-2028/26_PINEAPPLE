...
function getOwnProfile() {
  const fullName =
    String(
      STATE?.userProfile?.fullName || STATE?.currentUser?.name || "TeamForge User",
    ).trim() || "TeamForge User";
  const currentRecord =
  ...
  const storedProjects = Array.isArray(currentRecord?.data?.projects) ? currentRecord.data.projects : [];
  
  const projectList = storedProjects.map((project) => ({
    name: project.name || project.projectName,
    role: (project.owner || project.ownerName) === fullName ? "Owner" : "Collaborator",
    status: project.isCompleted || project.status === "completed" ? "Completed" : "Active",
    contribution:
      (project.owner || project.ownerName) === fullName
        ? "Leading project delivery"
        : "Contributing to the team",
  }));
  
  const assignedTasks = storedProjects.reduce((count, project) => {
    const tasks = Array.isArray(project.tasks) ? project.tasks : [];
    // The stored tasks have `assignedTo` (email) or we can match `assignee` name if populated
    return (
      count +
      tasks.filter((task) => {
         return String(task.assignedTo || "").trim().toLowerCase() === String(currentRecord?.email || "").trim().toLowerCase() ||
                String(task.assignee || "").trim() === fullName;
      }).length
    );
  }, 0);
  ...
  return `
    ...
    <div class="card">
      <div class="profile-header">
        <div class="${avatarClass}">${p.avatarUrl ? `<img src="http://localhost:3000${p.avatarUrl}" style="width:100%; height:100%; border-radius:50%; object-fit:cover;">` : p.initials}</div>
        <div class="profile-info">
          <div class="profile-name">
            ${p.name} ${mentorBadge}
          </div>
          <div class="profile-title">${[p.title, p.uni].filter(Boolean).join(' · ')}</div>
...
function renderProfile(user) {
  ...
  if (isOther && OTHER_PROFILES[user.name]) {
    ...
    const otherRecord = Object.values(users).find(u => String(u?.name || "").trim().toLowerCase() === user.name.trim().toLowerCase());
    if (otherRecord?.profile) {
      p.xp = Number(otherRecord.profile.xp || 0);
      p.rep = Number(otherRecord.profile.rep || 0);
    }
  } else {
    p = getOwnProfile();
