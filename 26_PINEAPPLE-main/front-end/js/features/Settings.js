...
function renderSettings() {
  ...
  if (notifContainer) {
    notifContainer.innerHTML = notifToggles.map((t) => makeToggle(t)).join("");
  }
  ...
  if (privacyContainer) {
    privacyContainer.innerHTML = privacyToggles.map((t) => makeToggle(t)).join("");
  }
}
...
function getProfileIdentity(profileInput = STATE.userProfile) {
  const profile = profileInput || {};
  const fullName =
    String(
      profile.fullName ||
        STATE.currentUser?.name ||
        getCurrentSettingsUserRecord()?.name ||
        "TeamForge User",
    ).trim() || "TeamForge User";
...
function hydrateProfileData() {
  ...
  const accountName = String(stored?.fullName || userRecord?.name || "").trim();
  const runtimeName = String(base.fullName || STATE.currentUser?.name || "").trim();
  const currentName = accountName || runtimeName || "TeamForge User";
  const merged = {
    fullName: String(stored?.fullName || userRecord?.name || base.fullName || currentName).trim() || currentName,
    username:
      String(
        stored?.username ||
          base.username ||
...
function applyProfileIdentityToUI() {
  const profile = STATE.userProfile || hydrateProfileData();
  const { fullName, displayName } = getProfileIdentity(profile);
  ...
  if (userHeader) userHeader.textContent = displayName;
  ...
  if (dashboardSubtitle) {
    ...
    dashboardSubtitle.innerHTML = `Welcome back, ${escapeSettingsHtml(displayName)}!${mentorBadgeHtml}`;
  }
...
function renderSkills() {
  ...
  if (!container) return;
  if (!STATE.userSkills) STATE.userSkills = [];
  container.innerHTML = STATE.userSkills
    .map(
      (s) => {
        const encodedSkill = encodeURIComponent(String(s || ""));
        return `
    <span class="badge badge-secondary" style="gap:5px;padding:4px 10px;font-size:.78rem">
      ${escapeSettingsHtml(s)}
      <button onclick="removeSkill('${encodedSkill}')" style="background:none;border:none;cursor:pointer;color:var(--muted-fg);font-size:.9rem;line-height:1;padding:0">&times;</button>
    </span>
  `;
      },
    )
...
function removeSkill(skill) {
  const normalizedSkill = decodeURIComponent(String(skill || ""));
  STATE.userSkills = STATE.userSkills.filter((s) => s !== normalizedSkill);
  if (typeof syncRuntimeProfileToAuthStore === "function") {
    syncRuntimeProfileToAuthStore();
  }
  if (typeof saveUserRuntime === "function") {
...
function applyTheme(val, saveToStorage = false) {
  ...
  if (saveToStorage) {
    if (mode === "light") {
      localStorage.removeItem("teamforge.theme");
    } else {
...
function saveProfileSettings() {
  ...
  if (bio && (bio.length < 10 || bio.length > 180)) {
    showToast("Bio must be 10 to 180 characters when provided");
    return;
  }
  ...
  if (typeof renderDashboard === "function") {
    const activeDashboardPage = document.getElementById("page-dashboard");
    if (activeDashboardPage?.classList.contains("active")) {
      renderDashboard();
      applyProfileIdentityToUI();
    }
  }
