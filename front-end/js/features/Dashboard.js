...
function renderDashboard() {
  if (typeof applyProfileIdentityToUI === "function") {
  ...
  const mentorRequests = isMentor && Array.isArray(PROJECTS)
    ? PROJECTS
        ...
        .filter(Boolean)
    : [];
  const visibleProjects = Array.isArray(currentUserData.projects) ? currentUserData.projects : [];
  ...
  const completedTasksCount = visibleProjects.reduce((count, project) => {
    const runtime =
      typeof getProjectRuntime === "function"
        ? getProjectRuntime(project)
        : project?.runtime;
    ...
    return (
      ...
      tasks.filter((task) => {
        ...
        const assigneeName = String(
          task?.assignee || task?.assignedToName || "",
        )
          .trim()
          .toLowerCase();
        const assigneeEmail = String(task?.assignedTo || "")
          .trim()
          .toLowerCase();

        return (
          (assigneeName && assigneeName === normalizedCurrentUserName) ||
          (assigneeEmail && assigneeEmail === normalizedCurrentUserEmail)
        );
      }).length
    );
  }, 0);
  ...
  document.getElementById("activity-list").innerHTML = actData
    ...
    <div class="activity-item">
      ...
      <div class="activity-text">
        <div class="activity-title">${escapeDashboardHtml(a.action)}</div>
        <div class="activity-sub">${escapeDashboardHtml(a.project)}</div>
      </div>
      <div class="activity-time">${escapeDashboardHtml(a.time)}</div>
    </div>
  `,
    )
    .join("");
      ...
      <div class="space-y-3">
        ${
          ...
          <div class="checkin-row">
            <div>
              <div class="font-semibold text-sm">${escapeDashboardHtml(request.projectName || request.projectId)}</div>
              <div class="text-xs text-muted">Owner: ${escapeDashboardHtml(request.owner)}</div>
            </div>
            <span class="badge ${
              request.status === "approved"
                ? "badge-success"
                : request.status === "declined"
                  ? "badge-destructive"
                  : "badge-warning"
            }">${escapeDashboardHtml(request.status === "requested" ? "pending" : request.status)}</span>
          </div>
        `,
                )
                .join("")
    ...
    sc.innerHTML = `<div class="card-title">My Skills</div>
      ${
        skills.length === 0
          ...
          : `<div style="display:flex;flex-wrap:wrap;gap:7px">${skills.map((s) => `<span class="badge badge-secondary">${escapeDashboardHtml(s)}</span>`).join("")}</div>`
      }`;
  }
  ...
  if (isMentor) {
      ...
      <div class="space-y-3">
        ${
          ...
          <div class="pending-row">
            <div>
              <div class="font-semibold text-sm">${escapeDashboardHtml(request.projectName || request.projectId)}</div>
              <div class="text-xs text-muted">from ${escapeDashboardHtml(request.owner)}</div>
            </div>
            <div class="flex gap-2 items-center">
              <span class="badge badge-secondary">${escapeDashboardHtml(currentUserEmail)}</span>
    ...
    const contributions = visibleProjects
      .slice(0, 5)
      .map((project) => {
        const runtime =
          typeof getProjectRuntime === "function"
            ? getProjectRuntime(project)
            : project?.runtime;
        const userEntry = Array.isArray(runtime?.contributionHistory)
          ? runtime.contributionHistory.find(
              (item) =>
                String(item?.by || "").trim().toLowerCase() ===
        ...
        return {
          ...
          contrib: String(userEntry.summary || "").trim(),
          evidence: "Open workspace",
          status: String(userEntry.status || "In Review").trim() || "In Review",
        };
      })
      .filter(Boolean);

    tc.innerHTML = `<div class="card-title">Contribution History</div>
      <div class="text-xs text-muted mb-2">${escapeDashboardHtml(currentUserName)} | ${escapeDashboardHtml(currentUserEmail)}</div>
            ...
            contributions.length
              ? contributions
                  .map(
                    (c) => `
            <tr>
              <td>${escapeDashboardHtml(c.project)}</td>
              <td class="text-muted">${escapeDashboardHtml(c.contrib)}</td>
              <td><span class="text-xs text-info">${dashboardIcon("link")} ${escapeDashboardHtml(c.evidence)}</span></td>
              <td><span class="badge ${String(c.status).toLowerCase() === "approved" ? "badge-success" : "badge-warning"}">${escapeDashboardHtml(c.status)}</span></td>
            </tr>
          `,
                  )
                  .join("")
              : '<tr><td colspan="4" class="text-sm text-muted">No contribution history available yet.</td></tr>'
          }</tbody>
        </table>
      </div>`;
  }
